...
int main(){
		...
		//CFBoard testBoard =
CFBoard("rnb1kbnr/ppp1pppp/4q3/8/8/2N5/PPPP1PPP/R1BQKBNR w KQkq - 2 4"); //
wrong legal moves for bishop
		// cout << board.naiveCheckCheck(0) << '\n';
		testBoard.movePiece(52, 36);
		std::cout << testBoard.getRepr() << std::endl;
		testBoard.movePiece(11, 27);
		std::cout << testBoard.getRepr() << std::endl;
		// std::cout << testBoard.getReprLegalMove(testBoard.getPieceFromCoords(36),
36) << std::endl; testBoard.movePiece(36, 27); std::cout << testBoard.getRepr()
<< std::endl; testBoard.movePiece(3, 27); std::cout << testBoard.getRepr() <<
std::endl; testBoard.movePiece(57,42); std::cout << testBoard.getRepr() <<
std::endl; testBoard.movePiece(27, 20); std::cout << testBoard.getRepr() <<
std::endl; std::cout << testBoard.getReprLegalMove(4, 61) << std::endl;
...
int main(){
		...
		CFBoard board =
CFBoard("rn1q1rk1/pnpp2pp/1p2p3/5p2/2PPP3/2P3P1/P1QN1P1P/R1B1K2R w KQ e6 0 1");
...
bool CFBoard::isCurrentBoardLegal() { return isStateLegal; }
...
void CFBoard::forceMovePiece(int startTile, int endTile,
														 int pawnPromotionType) {
	int piece = getPieceFromCoords(startTile);
	...
	// check whether the pawn reached point of promotion, if so promote it to
	// specified new piece type
	if (piece <= 1) {
		if (piece == 0) {
			if (endTile <= 7) {
				...
				if (pawnPromotionType == -1) {
					addPiece(8, endTile);
				} else if (pawnPromotionType % 2 == 0 &&
									 abs(pawnPromotionType - 5) <= 3) {
			...
			if (endTile >= 56) {
				...
				if (pawnPromotionType == -1) {
					addPiece(9, endTile);
				} else if (pawnPromotionType % 2 == 1 &&
									 abs(pawnPromotionType - 6) <= 3) {
					addPiece(pawnPromotionType, endTile);
				} else {
...
uint64_t CFBoard::getCardinals(int tile, bool color) {
	...
	uint64_t columnMap = (1ll + (1ll << 8) + (1ll << 16) + (1ll << 24) +
												(1ll << 32) + (1ll << 40) + (1ll << 48) + (1ll << 56))
											 << (tile & 7);
	...
	return ((((tile >> 3 << 3) != 0) *
					 ~((1ll << (63 - (__builtin_clzll(
																(((1ll << tile) - 1) & allBoard & columnMap) |
																(columnMap & (1 + ~columnMap))) &
														63))) -
						 1)) &
							(columnMap >> (64 - (tile >> 3 << 3))) |
					(((tile & 7) != 0) *
					 ~((1ll << (63 - (__builtin_clzll((((1ll << tile) - 1) & allBoard) |
																						(rowMap & (1 + ~rowMap))) &
														63))) -
						 1)) &
							(rowMap & ((1ll << tile) - 1)) |
					(tile != 63) * ((((allBoard & ~((1ll << (tile + 1)) - 1)) &
														(1 + (~(allBoard & ~((1ll << (tile + 1)) - 1)))))
													 << 1) -
															1 &
													(rowMap & ~((1ll << (tile + 1)) - 1))) |
					(tile != 63) *
							((((allBoard & ~((1ll << (tile + 1)) - 1) & columnMap) &
								 (1 + (~(allBoard & ~((1ll << (tile + 1)) - 1) & columnMap))))
								<< 1) -
									 1 &
							 (columnMap & ~((1ll << (tile + 1)) - 1)))) &
...
uint64_t CFBoard::getDiagonals(int tile, bool color) {
	...
	uint64_t slashMap = ((1ll << (8 * 0 + ((slashId - 0 - 1) & 7))) *
											 !((row < slashId) ^ (0 < slashId))) |
											((1ll << (8 * 1 + ((slashId - 1 - 1) & 7))) *
											 !((row < slashId) ^ (1 < slashId))) |
											((1ll << (8 * 2 + ((slashId - 2 - 1) & 7))) *
											 !((row < slashId) ^ (2 < slashId))) |
											((1ll << (8 * 3 + ((slashId - 3 - 1) & 7))) *
											 !((row < slashId) ^ (3 < slashId))) |
											((1ll << (8 * 4 + ((slashId - 4 - 1) & 7))) *
											 !((row < slashId) ^ (4 < slashId))) |
											((1ll << (8 * 5 + ((slashId - 5 - 1) & 7))) *
											 !((row < slashId) ^ (5 < slashId))) |
											((1ll << (8 * 6 + ((slashId - 6 - 1) & 7))) *
											 !((row < slashId) ^ (6 < slashId))) |
											((1ll << (8 * 7 + ((slashId - 7 - 1) & 7))) *
											 !((row < slashId) ^ (7 < slashId)));

	uint64_t bslashMap = ((1ll << (8 * 0 + ((bslashId + 0) & 7))) *
												!((7 - row < bslashId) ^ (7 - 0 < bslashId))) |
											 ((1ll << (8 * 1 + ((bslashId + 1) & 7))) *
												!((7 - row < bslashId) ^ (7 - 1 < bslashId))) |
											 ((1ll << (8 * 2 + ((bslashId + 2) & 7))) *
												!((7 - row < bslashId) ^ (7 - 2 < bslashId))) |
											 ((1ll << (8 * 3 + ((bslashId + 3) & 7))) *
												!((7 - row < bslashId) ^ (7 - 3 < bslashId))) |
											 ((1ll << (8 * 4 + ((bslashId + 4) & 7))) *
												!((7 - row < bslashId) ^ (7 - 4 < bslashId))) |
											 ((1ll << (8 * 5 + ((bslashId + 5) & 7))) *
												!((7 - row < bslashId) ^ (7 - 5 < bslashId))) |
											 ((1ll << (8 * 6 + ((bslashId + 6) & 7))) *
												!((7 - row < bslashId) ^ (7 - 6 < bslashId))) |
											 ((1ll << (8 * 7 + ((bslashId + 7) & 7))) *
												!((7 - row < bslashId) ^ (7 - 7 < bslashId)));

	return (((tile != 0) *
							 (~((1ll << (63 - (__builtin_clzll((((1ll << tile) - 1) &
																									allBoard & slashMap) |
																								 (slashMap & (1 + ~slashMap))) &
																 63))) -
									1)) &
					 (slashMap & ((1ll << tile) - 1))) |
					((tile != 0) * (~((1ll << (63 - (__builtin_clzll(
																							 (((1ll << tile) - 1) & allBoard &
																								bslashMap) |
																							 (bslashMap & (1 + ~bslashMap))) &
																					 63))) -
														1)) &
					 (bslashMap & ((1ll << tile) - 1))) |
					((tile != 63) *
					 ((((allBoard & ~((1ll << (tile + 1)) - 1) & bslashMap) &
							(1 + (~(allBoard & ~((1ll << (tile + 1)) - 1) & bslashMap))))
						 << 1) -
								1 &
						(bslashMap & ~((1ll << (tile + 1)) - 1)))) |
					((tile != 63) *
					 ((((allBoard & ~((1ll << (tile + 1)) - 1) & slashMap) &
							(1 + (~(allBoard & ~((1ll << (tile + 1)) - 1) & slashMap))))
						 << 1) -
								1 &
						(slashMap & ~((1ll << (tile + 1)) - 1))))) &
				 (~getColorBitBoard(color));
}
...
uint64_t CFBoard::getKnightPattern(int tile, bool color) {
	...
	return ((column > 0 && row > 1) * ((1ll << (tile - 17)) & ~allyBoard)) |
...
uint64_t CFBoard::getKingPattern(int tile, bool color) {
	...
	kingPattern =
			((1ll << (tile - 1)) * (column > 0) | (1ll << (tile - 8)) * (row > 0) |
			 (1ll << (tile + 1)) * (column < 7) | (1ll << (tile + 8)) * (row < 7) |
	...
	// If you customized the whole board into an illegal position, this part may
	// crash the code.
	uint64_t board = whiteBoard | blackBoard;
	if (castle >> 1) { // long
		bool longsideoccupied = ((board >> (tile - 1)) & 1) |
														((board >> (tile - 2)) & 1) |
														((board >> (tile - 3)) & 1);
		if (!longsideoccupied) {
			kingPattern += (1ll << (tile - 2));
		}
	}
	if (castle & 1) { // short
		bool shortsideoccupied =
				((board >> (tile + 1)) & 1) | ((board >> (tile + 2)) & 1);
		if (!shortsideoccupied) {
			kingPattern += (1ll << (tile + 2));
		}
	}
	...
	if (color) { // black
		...
		if (row == 7) { // just in case, should be useless in practice
			...
			if (!(((allyBoard | enemyBoard) >> (tile + 8)) & 1)) { // front
				...
				if ((row == 1) &&
						!(((allyBoard | enemyBoard) >> (tile + 16)) & 1)) { // frontfront
